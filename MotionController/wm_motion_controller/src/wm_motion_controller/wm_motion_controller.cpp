...
WmMotionController::WmMotionController()
    ...
    this->declare_parameter<float>("odometry_offset", odometry_offset_);
    this->get_parameter("odometry_offset", odometry_offset_);

    RCLCPP_INFO(this->get_logger(), "parameter test %f", odometry_offset_);
    ...
    cbg_rpm_ = create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
    ...
    rclcpp::SubscriptionOptions sub_rpm_options;
    ...
    sub_imu_ = this->create_subscription<sensor_msgs::msg::Imu>(constants_->tp_imu_, constants_->m_tp_queue_size,
                                                                std::bind(&WmMotionController::imu_callback, this, _1),
                                                                sub_imu_options);
    sub_mode_ = this->create_subscription<can_msgs::msg::Mode>(constants_->tp_control_mode_, 1,
                                                               std::bind(&WmMotionController::slam_mode_callback, this,
                                                                         _1), sub_controller_mode_options);

    sub_break_ = this->create_subscription<route_msgs::msg::DriveBreak>(constants_->tp_break_, 1,
                                                                        std::bind(&WmMotionController::break_callback,
                                                                                  this, _1), sub_break_options);
...
//		sub_break_ = this->create_subscription<route_msgs::msg::DriveBreak>("/drive/break",1,std::bind(&WmMotionController::break_callback,this,_1),sub_break_options);
    ...
    pub_rtt_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(constants_->tp_rtt_odom_, 10,
                                                                       pub_rtt_odom_options);
    pub_steering_ = this->create_publisher<can_msgs::msg::AdControlSteering>(constants_->tp_control_steering_, 1,
                                                                             pub_steering_options);
    pub_brake_ = this->create_publisher<can_msgs::msg::AdControlBrake>(constants_->tp_control_brake_, 1,
                                                                       pub_brake_options);
    pub_body_ = this->create_publisher<can_msgs::msg::AdControlBody>(constants_->tp_control_body_, 1, pub_body_options);
    pub_accelerate_ = this->create_publisher<can_msgs::msg::AdControlAccelerate>(constants_->tp_control_accelerate_, 1,
                                                                                 pub_steering_options);
    //
    timer_ = this->create_wall_timer(std::chrono::milliseconds(100),
                                     std::bind(&WmMotionController::pub_odometry, this));
    tf_timer_ = this->create_wall_timer(std::chrono::milliseconds(100),
                                        std::bind(&WmMotionController::update_transform, this));
    //
...
void WmMotionController::fn_recv_rpm(const float &rpm, const std::chrono::system_clock::time_point &cur_time) {
    ...
    converter->set_odometry_offset(odometry_offset_);
    //std::cout<<"wm_motion_controller.cpp"<<__LINE__<<std::endl;
...
void WmMotionController::pub_odometry() {
    ...
    if(eular<0){
        ...
        rtt_value.pose.orientation.y = eular;
    }
...
void WmMotionController::rpm_callback(const can_msgs::msg::TorqueFeedback::SharedPtr rpm) {
    //zRCLCPP_INFO(this->get_logger(),"speed %f",static_cast<float>(rpm->mcu_speed));
    //std::cout<<"wm_motion_controller.cpp"<<__LINE__<<std::endl;
    cur_ugv_->set_cur_rpm(static_cast<float>(rpm->mcu_speed));
    //std::cout<<"wm_motion_controller.cpp"<<__LINE__<<std::endl;
    cur_ugv_->set_cur_time(std::chrono::system_clock::now());
    //std::cout<<"wm_motion_controller.cpp"<<__LINE__<<std::endl;
    std::shared_ptr<CONVERTER::UGVConverter> converter = std::make_shared<CONVERTER::UGVConverter>();
    converter->set_odometry_offset(odometry_offset_);
    //RCLCPP_INFO(this->get_logger(),"rpm %f",static_cast<float>(prev_ugv_->get_cur_rpm()));
    cur_ugv_->set_cur_distance((*converter).rpm_to_distance(*prev_ugv_, *cur_ugv_));
    //std::cout<<"wm_motion_controller.cpp"<<__LINE__<<std::endl;
    //RCLCPP_INFO(this->get_logger(),"distance %.02f",cur_ugv_->get_cur_distnace());
    prev_ugv_->set_cur_rpm(cur_ugv_->get_cur_rpm());
    //std::cout<<"wm_motion_controller.cpp"<<__LINE__<<std::endl;
    prev_ugv_->set_cur_time(cur_ugv_->get_cur_time());
    //std::cout << "wm_motion_controller.cpp" << __LINE__ << std::endl;
