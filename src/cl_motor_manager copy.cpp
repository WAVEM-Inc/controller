...
void cl_motor_manager::test_read(){
	...
	//
	//std::thread thread_voltage_test(&cl_motor_manager::fn_voltage_request_test,this);
	//
	thread_read.join();
	thread_encoder.join();
...
void cl_motor_manager::msgCallback(const geometry_msgs::Twist& cmd_vel_msg){
	...
	*/
	...
	int led_red, led_blue,led_green=0;
	if(vel_linear==0&&vel_angular==0){
		led_red=1;
		led_blue=0;
		led_green=0;
	}
	else if(abs(vel_linear)>0.2||abs(vel_angular)>0.2){
		led_red=0;
		led_blue=1;
		led_green=0;
	}
...
void cl_motor_manager::fn_device_read_(){
	...
	while(true){
		...
		int data_start = fn_index_of(recv_data,'<',sizeof(char)*RECV_SIZE);
		std::cout<<"@@"<<data_start<<std::endl;
		right_angle_bracket=strrchr(recv_data,'>');
		//int data_end = fn_index_of(recv_data,'>',sizeof(char)*RECV_SIZE);
		if((right_angle_bracket-recv_data)-(left_angle_bracket-recv_data)==14){
			std::cout<<"Packet Read Success ";
			m_motor_recv_data.fn_set_left_move((std::atoi(fn_substr(recv_data,data_start+RECV_LEFT_START,data_start+RECV_LEFT_END))));
			...
			m_motor_recv_data.fn_set_right_move(std::atoi(fn_substr(recv_data,data_start+RECV_RIGHT_START, data_start+RECV_RIGHT_END)));
...
int cl_motor_manager::fn_index_of(char* str, char ch, int size){
		...
		while(ros::ok)
		{
			...
			if(l_sem_val < 1)
			{
				...
				if(read_left_flag==1 && l_ro_val!=0){
					...
					else if((std::abs(left_check)<MOTOR_ERROR_CHECK)||l_ro_val==0){
			...
			if(r_sem_val < 1)
			{
				...
				//	std::cout<<"[sema_test_r_old]"<<r_ro_val<<" "<<m_motor_recv_data.fn_get_old_right_move()<<std::endl;
					...
					else if((std::abs(right_check)<MOTOR_ERROR_CHECK||r_ro_val==0)){
...
void cl_motor_manager::fn_ros_run(){
	...
	ros::Subscriber ros_tutorial_sub = nh.subscribe(TP_CMD_VEL, 1, &cl_motor_manager::msgCallback);
	ros::spin();
}
