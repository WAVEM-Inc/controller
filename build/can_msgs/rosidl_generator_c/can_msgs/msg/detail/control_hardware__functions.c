  ...
  can_msgs__msg__ControlHardware__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    can_msgs__msg__ControlHardware * data =
      (can_msgs__msg__ControlHardware *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!can_msgs__msg__ControlHardware__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          can_msgs__msg__ControlHardware__fini(&output->data[i]);
        }
        return false;
      }
    }
